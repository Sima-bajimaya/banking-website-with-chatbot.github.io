...
def verify_customer(userid,user,status):
	...
	if type(status)!=int:
		status=int(status)
	sub="Verification of your Hamro bank account"
	if status==1:
		...
		if bit=="-99":
			...
			return "-22"
		cont="""<h1>Hamro Bank</h1>,<br/><p>Your account has approved for banking and using of our services.</p>""";
	else:
		cont="""<h1>Hamro Bank</h1>,<br/><p>We are sorry to hear that</p><br/></p><p>Your account is rejected.</p>""";
	conn,cursor=config.connect_to_database()
	try:
		cursor.execute(sql)
		m=send_mail.Mail()
...
def verify_card_request(r_id,acc_id,status):
	...
	if status==1:
		...
		if bit=="-99":
			...
			return "-22"
		cont="""<h1>Hamro Bank</h1>,<br/><p>Your card request has approved for banking and using of our services.</p>""";
	else:
		cont="""<h1>Hamro Bank</h1>,<br/><p>We are sorry to hear that</p><br/></p><p>Your account is rejected.</p>""";
	conn,cursor=config.connect_to_database()
	try:
