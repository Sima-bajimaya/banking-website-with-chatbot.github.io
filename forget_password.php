...
<div class="container well login_block" align="center">
	<div class="row">
		<div><caption><a href="index.php"><a href="index.php"><img src="images/banklogo.svg" class="img-responsive" style="margin-bottom: -30px; margin-top: -30px; width:220px;height:220px;float:center;"/></a></caption></div>
	</div>
...
$(document).ready(function(){
	...
	$("#varify").click(function(e){
		e.preventDefault();
		var otp2=$("#otp2").val();
		var email=document.myForm.forget_email.value;
		console.log(email)
		var x=new XMLHttpRequest();
		x.onreadystatechange=function(){
			if(this.readyState==4 && this.status==200)
				{
					var data=this.responseText.replace("\r\n","").trim();
					if(data==1){
						$("#email_sender").hide();
						$("#otp_handler").hide();
						$("#new_password").show();
		...
		x.open("POST","verify_otp.py",true);
		x.setRequestHeader("Content-type","application/x-www-form-urlencoded");
		x.send("otp="+otp2+"&email="+email);
		// if(otp == otp2)
		// 	{
		// 		$("#email_sender").hide();
		// 		$("#otp_handler").hide();
...
function send_OTP(s)
	{
		...
		var x=new XMLHttpRequest();
		x.onreadystatechange=function(){
			console.log(x.readyState,x.status,this.responseText,s)
