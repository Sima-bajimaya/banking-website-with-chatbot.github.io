...
def train_dialogue(domain_file="bank_domain.yml", model_path="models/dialogue/", training_data_file="data/stories.md"):
    ...
    fallback = FallbackPolicy(
        fallback_action_name="action_fallback", core_threshold=0.2, nlu_threshold=0.2)
    agent = Agent(domain_file, policies=[MemoizationPolicy(
        max_history=15), KerasPolicy(), fallback])
    training_data = agent.load_data(training_data_file)
    agent.train(training_data, batch_size=200,
...
def train_nlu():
    ...
    model_directory = trainer.persist(
        'models/nlu/', fixed_model_name="bank_nlu")
...
def run_nlu():
    ...
    #   "models/nlu/default/bank_nlu")
    # print (interpreter.parse(u"Transfer Money please"))
    # print (interpreter.parse(u"change my passcode"))
...
def run_bank_bot(serve_forever=True):
    from rasa_core.agent import Agent
    from rasa_core.interpreter import Interpreter
...
if __name__ == '__main__':
    ...
    parser.add_argument('task', choices=[
                        "train-nlu", "train-dialogue", "run"], help="what the bot should do - e.g. run or train?")
    ...
    if task == "train-nlu":
        ...
        warnings.warn(
            "Need to pass either 'train-nlu', 'train-dialogue' or 'run' to use the script.")
        exit(1)
    # run_nlu();