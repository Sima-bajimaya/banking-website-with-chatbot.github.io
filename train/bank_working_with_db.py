...
def get_personal_info(user,password,field):
    print("user and password is",user,password)
    f=check_indentity(user,password)
...
def send_forgot_mail(user):
    ...
    try:
        ...
        if num==1:
            ...
            otp_sql="update customers SET last_otp='%s' where email='%s'"%(otp, user)
            cursor.execute(otp_sql)
            conn.commit()
            return 1
        else:
            return -99
...
def verify_forget_otp(user,otp):
    user=protect_data(user)
    conn,cursor=connect_to_database()
    sql="select * from customers where email='%s' and last_otp='%s'"%(user,otp)
    try:
        cursor.execute(sql)
        num=cursor.rowcount
        if num==1:
