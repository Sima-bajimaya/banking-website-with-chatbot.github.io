...
from bank_working_with_db import *
import random,re
import security
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        if service_access != 1:
            dispatcher.utter_template(
                "utter_error_caught_reply", tracker, name=name)
        ...
        if hasAcc == -99:
            ...
            dispatcher.utter_message(
                "You do not have any account yet. You can apply for a card application after getting an Hamro Bank account.")
            return []
        balance = get_account_info(user, password, "balance")
        acc = random.choice(['account', 'a/c', 'acc'])
        dispatcher.utter_template(
            "utter_get_account_balance_reply", tracker, name=name, balance=balance, acc=acc)
        return [SlotSet("requested_slot", None), SlotSet("service_access", None)]
...
class ActionGetAccountDetails(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        if hasAcc == -99:
            ...
            dispatcher.utter_message(
                "You do not have any account yet. You can apply for a card application after getting an HamroBank Bank account.")
            return []
        dispatcher.utter_template(
            "utter_get_account_details_reply", tracker, name=name)
        return []
...
class ActionGetCardDetails(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        if cardExist == -22:
            dispatcher.utter_message(
                "You do not have any card yet, But you can apply for a card application.")
            return []
        elif cardExist == -99:
            dispatcher.utter_template(
                "utter_error_caught_reply", tracker, name=name)
            return []
        dispatcher.utter_template(
            "utter_get_card_details_reply", tracker, name=name)
        return []
...
class ActionGetCardStatus(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        if hasAcc == -99:
            ...
            dispatcher.utter_message(
                "You do not have any account yet. You can apply for a card application after getting an HamroBankS Bank account.")
            return []
        cardExist = customer_card_has(userid)
        if cardExist == -22:
            dispatcher.utter_message(
        ...
        if status == 1:
            ...
            return []
        dispatcher.utter_template(
            "utter_get_card_status_reply", tracker, name=name, status=status)
        return []
...
class ActionBotControlStartOver(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        if results != 1:
            ...
            return [ActionReverted(), AllSlotsReset()]
        dispatcher.utter_template(
            "utter_bot_control_start_over_reply", tracker, name=name)
...
class ActionBotControlStandby(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        if results != 1:
            ...
            return [ActionReverted(), AllSlotsReset()]
        dispatcher.utter_template(
            "utter_bot_control_standby_reply", tracker, name=name)
        return []
...
class ActionBotControlConfirmPresence(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        if results != 1:
            ...
            return [ActionReverted(), AllSlotsReset()]
        dispatcher.utter_template(
            "utter_bot_control_confirm_presence_reply", tracker, name=name)
        return []
...
class ActionBotControlChangeSubject(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        if results != 1:
            ...
            return [ActionReverted(), AllSlotsReset()]
        dispatcher.utter_template(
            "utter_bot_control_change_subject_reply", tracker, name=name)
        return []
...
class ActionAskInputTransferMoney(FormAction):
    ...
    def submit(self, dispatcher, tracker, domain):
        user, password = tracker.get_slot(
            "email"), tracker.get_slot("password")
        access = tracker.get_slot("access")
        if user == None or password == None or access != 1:
            dispatcher.utter_message(
                "Please log in our service, to use HamroBank service!")
            return [ActionReverted(), AllSlotsReset()]
        name = tracker.get_slot("name")
        name = get_user_name(name)
        userid = get_personal_info(user, password, "cid")
        ...
        if amount > 50000:
            dispatcher.utter_message(
                "Please enter valid amount less than Rs.50000: ")
            return [SlotSet("amount", None), SlotSet("requested_slot", "amount")]
...
class ActionTransferMoney(FormAction):
    ...
    def submit(self, dispatcher, tracker, domain):
        ...
        if not result_where.startswith("11"):
            dispatcher.utter_message(
                "Transaction Failed! Account number was not correct.")
            return [SlotSet("last_otp", None), SlotSet("got_otp", None), SlotSet("transfer_perm", None), SlotSet("where", None), SlotSet("amount", None), SlotSet("service_access", None), SlotSet("requested_slot", None)]
        to_acc_no = where
        print("to account is: ", to_acc_no)
        amount = str(amount)
        print("Got aamount to transfer:", amount)

        if amount == None or (not re.match("[0-9]+", amount)):
            dispatcher.utter_message(
                "Transaction Failed! Amount was incorrect!")
            return [SlotSet("last_otp", None), SlotSet("got_otp", None), SlotSet("transfer_perm", None), SlotSet("where", None), SlotSet("amount", None), SlotSet("service_access", None), SlotSet("requested_slot", None)]
        amount = int(amount)
        if amount > 50000:
            dispatcher.utter_message(
        ...
        print(last_otp,"this is last otp")
        ...
        if (not re.match("[0-9]+", got_otp)):
            dispatcher.utter_message(
                "Transaction Failed! OTP was not correct.")
            return [SlotSet("last_otp", None), SlotSet("got_otp", None), SlotSet("transfer_perm", None), SlotSet("where", None), SlotSet("amount", None), SlotSet("service_access", None), SlotSet("requested_slot", None)]
        got_otp = int(got_otp)
        if got_otp == 1:
        ...
        if result_transfer == "-22":
            dispatcher.utter_message(
                "Transaction Failed! You have inefficient money.")
            return [SlotSet("last_otp", None), SlotSet("got_otp", None), SlotSet("transfer_perm", None), SlotSet("where", None), SlotSet("amount", None), SlotSet("service_access", None), SlotSet("requested_slot", None)]
            ...
            return [SlotSet("last_otp", None), SlotSet("got_otp", None), SlotSet("transfer_perm", None), SlotSet("where", None), SlotSet("amount", None), SlotSet("service_access", None), SlotSet("requested_slot", None)]
        dispatcher.utter_template(
            template, tracker, name=name, to_acc_name=acc_name)
        ans = get_last_transaction_without_html_tags(from_acc_no, 1)
        print("Transaction answer is:", ans)
        dispatcher.utter_template(
            "utter_view_activity_reply", tracker, name=name, ans=ans)
        return [SlotSet("last_otp", None), SlotSet("got_otp", None), SlotSet("transfer_perm", None), SlotSet("where", None), SlotSet("amount", None), SlotSet("service_access", None), SlotSet("requested_slot", None)]
...
class ActionChangeContact(FormAction):
    ...
    def submit(self, dispatcher, tracker, domain):
        ...
        if user == None or password == None or access != 1:
            dispatcher.utter_message(
                "Please log in our service, to use Hamro Bank service!")
            return [ActionReverted(), AllSlotsReset()]
        service_access = tracker.get_slot("service_access")
        name = tracker.get_slot("name")
        name = get_user_name(name)
        ...
        if result == -11:
            dispatcher.utter_message(
                "This contact number is already registered with another account")
        ...
        if result != 1:
            ...
            return [SlotSet("contact", None), SlotSet("requested_slot", None), SlotSet("service_access", None)]
        dispatcher.utter_template(
            "utter_change_entity_reply", tracker, name=name, ent="contact")
        return [SlotSet("contact", None), SlotSet("requested_slot", None), SlotSet("service_access", None)]
...
class ActionChangePasscode(FormAction):
    ...
    def submit(self, dispatcher, tracker, domain):
        ...
        if not re.match("[0-9]{6}", new_passcode):
            dispatcher.utter_message(
                "Passcode should not contain character and it should be of 6 numbers.\nPlease enter valid Passcode:\n")
        ...
        if result != 1:
            ...
            return [SlotSet("new_passcode", None), SlotSet("passcode", None), SlotSet("requested_slot", None), SlotSet("service_access", None)]
        dispatcher.utter_template(
            "utter_change_entity_reply", tracker, name=name, ent="passcode")
...
class ActionChangePassword(FormAction):
    ...
    def submit(self, dispatcher, tracker, domain):
        ...
        if not re.match("^(((?=.*[a-z])(?=.*[A-Z]))|((?=.*[a-z])(?=.*[0-9]))|((?=.*[A-Z])(?=.*[0-9])))(?=.{6,})", new_password):
            dispatcher.utter_message(
                "Password should contain at least one number and at least one character. It also have at least length of 6 characters.\n Please enter new password:")
        ...
        if result != 1:
            ...
            return [SlotSet("requested_slot", None), SlotSet("service_access", None)]
        dispatcher.utter_template(
            "utter_change_entity_reply", tracker, name=name, ent="password")
        return [AllSlotsReset()]
...
class ActionGetOTPPermission(FormAction):
    ...
    def submit(self, dispatcher, tracker, domain):
        ...
        if user == None or password == None or access != 1:
            dispatcher.utter_message(
                "Please log in our service, to use Bank service!")
            return [ActionReverted(), AllSlotsReset()]
        user, password, got_otp = tracker.get_slot("email"), tracker.get_slot(
            "password"), tracker.get_slot("got_otp")
        last_otp = tracker.get_slot("last_otp")
        last_otp = int(last_otp)
...
class ActionChangeUsername(FormAction):
    ...
    def submit(self, dispatcher, tracker, domain):
        ...
        if result != 1:
            ...
            return [SlotSet("username", None), SlotSet("requested_slot", None), SlotSet("service_access", None)]
        dispatcher.utter_template(
            "utter_change_entity_reply", tracker, name=name, ent="username")
        return [SlotSet("username", None), SlotSet("requested_slot", None), SlotSet("service_access", None)]
...
class ActionChangeAddress(FormAction):
    ...
    def submit(self, dispatcher, tracker, domain):
        ...
        dispatcher.utter_template(
            "utter_change_entity_reply", tracker, name=name, ent="postal address")
        return [SlotSet("address1", None), SlotSet("address2", None), SlotSet("requested_slot", None), SlotSet("service_access", None)]
...
class ActionChangeCredentialInfo(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        dispatcher.utter_template(
            "utter_change_credential_info_reply", tracker, name=name, ent=ent)
...
class ActionCardRequest(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        if cardExist == -99:
            ...
            dispatcher.utter_message(
                "You already have a card. If you have lost your card you can cancel your card and replace your card.")
            return []
...
class ActionGetContact(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        dispatcher.utter_template(
            "utter_got_asked_reply", tracker, name=name, key="contact number", value=contact)
        return []
...
class ActionGetEmail(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        dispatcher.utter_template(
            "utter_got_asked_reply", tracker, name=name, key=key, value=email)
        return []
...
class ActionGetUsername(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        dispatcher.utter_template(
            "utter_got_asked_reply", tracker, name=name, key="username", value=username)
        return []
...
class ActionGetAccountNumber(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        acc_no = get_account_info(user, password, "acc_no")
        dispatcher.utter_template(
            "utter_got_asked_reply", tracker, name=name, key="account number", value=acc_no)
        return []
...
class ActionGetCardNumber(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        dispatcher.utter_template(
            "utter_got_asked_reply", tracker, name=name, key="card number", value=card_no)
        return []
...
class ActionGetAddress(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        dispatcher.utter_template(
            "utter_got_asked_reply", tracker, name=name, key="postal address", value=postal_add)
        return []
    ...
    def submit(self, dispatcher, tracker, domain):
        ...
        if user == None or password == None or access != 1:
            dispatcher.utter_message(
                "Please log in our service, to use HamroBank service!")
            return [ActionReverted(), AllSlotsReset()]
        user, password, passcode = tracker.get_slot("email"), tracker.get_slot(
            "password"), tracker.get_slot("passcode")
        if passcode == None:
            return [ActionReverted()]
...
class ActionNegativeFeedback(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        name = get_user_name(name)
        dispatcher.utter_template(
            "utter_negative_feedback_reply", tracker, name=name)
        return []
...
class ActionPositiveFeedback(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        name = get_user_name(name)
        dispatcher.utter_template(
            "utter_positive_feedback_reply", tracker, name=name)
        return []
...
class ActivateCardService(FormAction):
    ...
    def submit(self, dispatcher, tracker, domain):
        ...
        if results == 1:
            ...
            dispatcher.utter_template(
                template, tracker, card_type=card_type.capitalize(), using_what=using_what)
        else:
            dispatcher.utter_message("Please enter valid passcode: ")
            return [SlotSet("passcode", None), SlotSet("requested_slot", "passcode")]
...
class GetFeeInquiry(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        if cardExist == -22:
            dispatcher.utter_message(
                "You do not have any card yet, But you can apply for a card application. You can always check status for card application in your Personal Settings Panel or you could ask me which will be easy for you i guess!")
            return []
        elif cardExist == -99:
            dispatcher.utter_template(
                "utter_error_caught_reply", tracker, name=name)
        ...
        fees = round(fees, 2)
        dispatcher.utter_template(
            "utter_fee_inquiry_reply", tracker, fees=fees)
        return []
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        if cardExist == -22:
            ...
            if res != 1:
                ...
                return []
            dispatcher.utter_message(
                "You do not have any card yet, But as i can see you already have raised a request for card application.")
            return [SlotSet("passcode", None), SlotSet("card_perm", None), SlotSet("card_replace_with", None), SlotSet("requested_slot", None), SlotSet("service_access", None)]
        elif cardExist == -99:
            dispatcher.utter_template(
                "utter_error_caught_reply", tracker, name=name)
